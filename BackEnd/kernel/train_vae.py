...
import time
import importlib
from model.vae import VAE, idx2onehot
...
parser.add_argument('--trainshadowmodel',action='store_true',default=True)
parser.add_argument('--setup',type=str,required=True,help="means the output path of purifier")
...
def train_purifier_with_helper(opt, purifier, batchsize, log_interval, device,
    ...
    for batch_idx, (data, target, raw) in enumerate(data_loader):
        ...
        loss = loss_function(prob_out_softmax, data_softmax, mu, log_var) #errG_3
...
def trainPurifierModel(opt,size,training_idx,repeat):
    ...
    basedir = './{}/{}/'.format(opt.dataset,opt.setup)
    ...
    dataset_train = dataset.LOAD_DATASET('../data', transform=transform,
                                         db_idx=training_idx,repeat_flag=False,D2_size=size)
    if len(dataset_train) != size:
        raise Exception('Invalid config!')
    
    # dataset_train = dataset.LOAD_DATASET('../data', transform=transform, db_idx=8, index=True, group=2)
    # dataset_test1 = dataset.LOAD_DATASET('../data', transform=transform, db_idx=8, index=True, group=1)
    # dataset_test2 = dataset.LOAD_DATASET('../data', transform=transform, db_idx=8, index=True, group=3)
    dataset_test1 = dataset.LOAD_DATASET('../data', transform=transform,
                                        db_idx=7) if opt.trainshadowmodel else dataset.LOAD_DATASET('../data',
                                                                                                    transform=transform,
                                                                                                    db_idx=0)
    dataset_test2 = dataset.LOAD_DATASET('../data', transform=transform,
    ...
    if opt.dataset == 'purchase':
        hidden_sizes = [128, 256, 512]
        latent_size = 20
    elif opt.dataset == 'facescrub':
    ...
    if opt.trainshadowmodel:
        ...
        path = os.path.join(opt.dataset, opt.dataset + '_targetmodel.pth')
...
def main():
    ...
    f = open("config/vae.json", encoding="utf-8")
    content = json.loads(f.read())
