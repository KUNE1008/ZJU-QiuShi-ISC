...
class LOAD_DATASET(Dataset):
    def __init__(self, root, transform=None, target_transform=None, db_idx=0, repeat_flag=False, as_train_data=False, index=False,as_out=False, group=1):
        ...
        # for D1 to D3
            ...
            if db_idx < 6:
                input2 = np.load(os.path.join(self.root, 'location/location_withoutdefence_D3.npz'))['data']
                if db_idx == 4:
                ...
                input2 = np.load(os.path.join(self.root, 'location/location_withoutdefence_D4.npz'))['data']
                if db_idx == 6: 
                    self.data = input2[datasize[0]:int(0.25*datasize[1])]
